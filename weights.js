...
export function recomputeDeltas(entries) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    sorted[i].delta = round2(sorted[i].weight - sorted[i - 1].weight);
  }
  return sorted;
}

const MS_PER_DAY = 864e5;
export const PROJECTED_RATE_PER_WEEK = 0.85; // kg lost per 7 days (iteration-2 75.0 kg goal, amended 2026-09-02; was 0.5)
...
export function previewDelta(entries, { date, weight }) {
  ...
  return { delta: round2(weight - prev.weight), vsDate: prev.date };
}
...
export function validateEntry({ date, weight }, entries, today) {
  ...
  if (typeof weight !== 'number' || !Number.isFinite(weight)) errors.push('Enter a weight.');
    ...
    if (round2(weight) !== weight) errors.push('Weight can have at most two decimals.');
  }
  return errors;
}
